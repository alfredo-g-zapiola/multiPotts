...
// creates a random matrtix for the initialization of z 
...
// returns a vector of samples from gamma with parameters shape and rate specifie as vectors
...
// returns a vector with samples from a normal with mu and sd specified in vecors
...
// returns a matrix that on column[i] has the pdf of a normal distribution of mean[i], sd[i] in y[j] we pass yunique and ymatch to avoid replicating calculations
...
// returns the value of the pdf of a multivariate normal distribution in x 
double dmdnorm(arma::vec x,  
              arma::vec mean,  
              arma::mat sigma, 
              bool logd = false) { 
  int xdim = x.n_elem;
...
// returns a matrix that on column[i] has the pdf of a normal distribution of mu[i], sigma[i] in y[j] we pass yunique and ymatch to avoid replicating calculations
arma::mat dnorm_field(const arma::mat & Y, const arma::mat & mus,
                const arma::cube & lambdas)
{
  arma::vec prob(Y.n_cols);
  arma::mat probMx(Y.n_cols,mus.n_cols);
  for (unsigned i=0; i<mus.n_cols; i++)
  {
    for (int j=0; j<Y.n_cols; j++)
    {
      prob[j] =  dmdnorm(Y.col(j),mus.col(i),lambdas.slice(i).i());
    }
    probMx.col(i) = prob;
  }
  return probMx;
                       ...
                       const arma::cube & pr_sigma_v0, const arma::rowvec & pr_sigma_n0, const arma::mat & mu)
{
  ...
  for (unsigned j=0; j < nZ.n_elem; j++)
  {
    if(nZ(j)!=0){
      ...
      arma::mat vp = pr_sigma_v0.slice(j) + A.i();
...
SEXP mdgibbsPotts(SEXP yS, SEXP betaS, SEXP muS, SEXP sigmaS, SEXP nS, SEXP bS, SEXP prS, SEXP itS) {
  BEGIN_RCPP
  arma::mat Y = Rcpp::as<arma::mat>(yS); //directly creating arma matrix/cube from SEXP
  ...
  arma::cube pr_sigma = Rcpp::as<arma::cube>(prR["sigma"]);
  arma::cube pr_sigma_v0 = Rcpp::as<arma::cube>(prR["sigma.B0"]);
  ...
  return Rcpp::List::create(
    Rcpp::Named("alloc") = alloc,    // count of allocations to each component
    Rcpp::Named("mu")    = mu_save,       // sample of mu
    Rcpp::Named("lambdas") = lambdas_save,       // sample of lambdas = inv(sigmas)
    Rcpp::Named("z")     = z,        // final sample from Gibbs distribution
    Rcpp::Named("sum") = sum_save,    // sum of identical neighbours
    Rcpp::Named("e") = e, // allocation vector
    Rcpp::Named("ne") = ne // counts of like neighbours
