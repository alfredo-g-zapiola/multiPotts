---
title: "Glicam Multidimensional GMM"
...
```
 ...
 we can get 96% of the explained variance with the first 4 components
...
plot(clusteringGMM, border=NA,asp = TRUE,col= rocket(k),axis.col=NULL, axis.row=NULL, xlab='', ylab='',key = NULL)
```
...
```{r}
plot(pcascore1vec,pcascore3vec,col = clusteringGMM[pixels[,1],pixels[,2]])
```

```{r}
plot(pcascore2vec,pcascore3vec,col = clusteringGMM[pixels[,1],pixels[,2]])
```

```{r}
plot(pcascore1vec,pcascore4vec,col = clusteringGMM[pixels[,1],pixels[,2]])
```

```{r}
plot(pcascore2vec,pcascore4vec,col = clusteringGMM[pixels[,1],pixels[,2]])
```

```{r}
plot(pcascore3vec,pcascore4vec,col = clusteringGMM[pixels[,1],pixels[,2]])
```
