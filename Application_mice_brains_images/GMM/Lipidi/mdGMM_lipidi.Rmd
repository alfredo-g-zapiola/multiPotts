---
title: "Multidimensional GMM lipidi"
...
plot(clusteringGMM, border=NA,asp = TRUE,col= turbo,axis.col=NULL, axis.row=NULL, xlab='', ylab='',key = NULL)
```
...
for( i in 1:iter){
  trace1[i] = sum(diag(resGMM$sigmas[i][[1]][,,1]))
  trace2[i] = sum(diag(resGMM$sigmas[i][[1]][,,2]))
  trace3[i] = sum(diag(resGMM$sigmas[i][[1]][,,3]))
}
...
for( i in 1:iter){
  ...
  trace2[i] = sum(diag(resGMM$sigmas[i][[1]][,,2]))
  trace3[i] = sum(diag(resGMM$sigmas[i][[1]][,,3]))
  trace4[i] = sum(diag(resGMM$sigmas[i][[1]][,,4]))
  trace5[i] = sum(diag(resGMM$sigmas[i][[1]][,,5]))
  trace6[i] = sum(diag(resGMM$sigmas[i][[1]][,,6]))
}
...
mu5chain = mcmc(t(resGMM$mu[,5,]))
mu6chain = mcmc(t(resGMM$mu[,6,]))
mu7chain = mcmc(t(resGMM$mu[,7,]))
mu8chain = mcmc(t(resGMM$mu[,8,]))
mu9chain = mcmc(t(resGMM$mu[,9,]))
```
...
plot(mu6chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(mu6chain,auto.layout = FALSE)
plot(mu7chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(mu7chain,auto.layout = FALSE)
plot(mu8chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(mu8chain,auto.layout = FALSE)
plot(mu9chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(mu9chain,auto.layout = FALSE)
```
...
for( i in 1:iter){
  ...
  trace5[i] = sum(diag(resGMM$sigmas[i][[1]][,,5]))
  trace6[i] = sum(diag(resGMM$sigmas[i][[1]][,,6]))
  trace7[i] = sum(diag(resGMM$sigmas[i][[1]][,,7]))
  trace8[i] = sum(diag(resGMM$sigmas[i][[1]][,,8]))
  trace9[i] = sum(diag(resGMM$sigmas[i][[1]][,,9]))
}
...
tr5chain = mcmc(trace5)
tr6chain = mcmc(trace6)
tr7chain = mcmc(trace7)
tr8chain = mcmc(trace8)
tr9chain = mcmc(trace9)
...
plot(tr6chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(tr6chain,auto.layout = FALSE)
plot(tr7chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(tr7chain,auto.layout = FALSE)
plot(tr8chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(tr8chain,auto.layout = FALSE)
plot(tr9chain,density = FALSE,auto.layout = FALSE)
autocorr.plot(tr9chain,auto.layout = FALSE)
```

## sparse mixture experiment
