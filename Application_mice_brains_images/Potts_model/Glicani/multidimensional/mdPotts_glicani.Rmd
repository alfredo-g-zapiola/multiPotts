...
iter = 10000
burnin = 5000
results <- GibbsPotts(data2,betacritic,initmu,initsigma,neigh,block,priors,iter,burnin)
```
...
for( i in 1:iter){
  trace1[i] = sum(diag(results$sigma[i][[1]][,,1]))
  trace2[i] = sum(diag(results$sigma[i][[1]][,,2]))
  trace3[i] = sum(diag(results$sigma[i][[1]][,,3]))
}
...
```
we dont have the problem of the beta critic
...
for( i in 1:iter){
  ...
  trace2[i] = sum(diag(results$sigma[i][[1]][,,2]))
  trace3[i] = sum(diag(results$sigma[i][[1]][,,3]))
  trace4[i] = sum(diag(results$sigma[i][[1]][,,4]))
  trace5[i] = sum(diag(results$sigma[i][[1]][,,5]))
  trace6[i] = sum(diag(results$sigma[i][[1]][,,6]))
}
...
autocorr.plot(betachain)
```
same again no problem with the beta
